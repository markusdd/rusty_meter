...
pub fn show_line_graph(
    ...
    plot.show(ui, |plot_ui| {
        ...
        let new_bounds = egui_plot::PlotBounds::from_min_max(
            [0.0, current_bounds.min()[1]],
            [mem_depth as f64, current_bounds.max()[1]],
        );
...
pub fn show_histogram(
    ...
    plot.show(ui, |plot_ui| {
        if !hist_values_vec.is_empty() {
            ...
            let y_bounds = [0.0, max_count * 1.1];
    ...
    ui.horizontal(|ui| {
        ...
        if ui
            ...
            .clicked()
        {
            *hist_collect_active = !*hist_collect_active; // Fixed: Single dereference to toggle
        }
    ...
    ui.horizontal(|ui| {
            ...
            .changed()
        {
            if let Ok(new_interval) = interval_str.parse::<u64>() {
                if new_interval > 0 {
                    *hist_collect_interval_ms = new_interval; // Fixed: Single dereference for assignment
                }
            }
        }
    });

    ui.label(format!("Histogram Memory Depth: {}", hist_mem_depth));
}
    ...
    pub fn update_histogram(&mut self, meas: f64) {
        if !meas.is_nan() && self.hist_collect_active {
            let current_time = std::time::SystemTime::now()
                ...
                .unwrap()
                .as_secs_f64();
            let hist_interval = self.hist_collect_interval_ms as f64 / 1000.0;
